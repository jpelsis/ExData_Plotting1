...
ts_rpwr <- as.ts(pwrdata$Global_reactive_power)
ts_volt <- as.ts(pwrdata$Voltage)
...
plot(ts_apwr,xaxt="n",ylab="Global Active Power",xlab="")
axis(side=1,at=c(1,length(ts_apwr)*0.5,length(ts_apwr)),labels=c("Thu","Fri","Sat"),xlab="")

plot(ts_volt,xaxt="n",ylab="Voltage",xlab="datetime")
axis(side=1,at=c(1,length(ts_volt)*0.5,length(ts_volt)),labels=c("Thu","Fri","Sat"),xlab="")
...
legend("topright",legend = c(names(pwrdata[,7:9])),col=c("black","red","blue"),lty=1,bty="n")

plot(ts_rpwr,xaxt="n",ylab="Global_reactive_power",xlab="datetime")
axis(side=1,at=c(1,length(ts_rpwr)*0.5,length(ts_rpwr)),labels=c("Thu","Fri","Sat"),xlab="")
